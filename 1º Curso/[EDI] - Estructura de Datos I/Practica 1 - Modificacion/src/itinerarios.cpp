...
void itinerarios::MostrarITI(Hora h, cadena dest){

    if(cargado){
        int aux=0;
        int hora=0;

        //primero emtemos la hora como un entero
        if(h.am==0)
            hora=1*10000; //es pm
        //else
            //hora=0*10000;  NO NECESARIO AL SER 0
        hora=hora+(h.h)*100+h.m;

            for(int pos=0;pos<n;pos++){

                //gurdamos la hora del elemento en el q nos encontramos
                aux=(elementos[pos].inicio.h)*100;

                aux=aux+elementos[pos].inicio.m;

                if (elementos[pos].inicio.am==0)
                    aux=aux+1*10000; //si es pm se suma sino no

                if(dest==elementos[pos].destino && hora<aux){
                        Mostrar("","",pos);
                }
            }
    }
    else
        cout<<"ITINERARIOS NO CARGADOS";
}
