...
void autobuses::Insertar(autobus a){ //Insertar=> solicita los datos del autobús y lo inserta en la tabla dinámica o fichero,
    ...
    if(cargado){ //fichero no cargado en memoria
        ...
        if(!existe){
            comp=strcmp(a.matricula,elementos[n-1].matricula);
            autobus aux;
            if(comp>0){
                elementos[n]=a;
                n++;
                ...
                /*for (int pos=0; pos<n-1; pos++){ //contador que calcula que se haga tantas vees como elemento tiene
                    //fin es la posición final valida (n-1)
                        for (i=n-1; i>pos; i--){
                            if(strcmp(elementos[i-1].matricula,elementos[i].matricula)>0){
                                aux=elementos[i-1];
            ...
            if(existe!=-1){
                ...
                while(!encontrado && i<n-1){
                    autobus aux;
                    fichero.seekg((-1)*sizeof(autobus),ios::cur); //al final para insertar
                    fichero.read((char *)&aux, sizeof (autobus)); //escribe el autobus introducido en el fichero
                    comp=strcmp(a.matricula,aux.matricula);
                    if(comp>0){
                        fichero.seekg((-1)*sizeof(autobus),ios::end);
                        for(int j=n-2;j>i;j--){
                            fichero.read((char*)&aux,sizeof(autobus));
                            fichero.seekp(0,ios::cur);
                            fichero.write((char*)&aux,sizeof(autobus));
                            fichero.seekg((-3)*sizeof(autobus),ios::cur);
                        }
                        fichero.seekp(0,ios::cur);
                ...
                if(i==n-1){ //si sale del bucle porq no se ha encontrado alguno menor es porq es la mayor y se inserta en el ultimo
                    elementos[n-1]=a;
                }
                fichero.close();
            }
