...
//MODIFICACIÓN 1
bool ColaPri::busqueda(cadena nombre, cadena apellido, int &prioridad){
/*Implementar un nuevo método de la clase ColaPri llamado Busqueda, que reciba
como parámetros, los nombres y apellidos de un paciente y devuelva si el
paciente está o no en la cola con prioridad y en el caso de que esté, en qué cola,
de qué prioridad, se encuentra.*/
    ColaPri aux;
    bool encontrado=false;
    int i;
    Paciente pac;
    if(!esvaciaColaPri()){
        while(!esvaciaColaPri() && !encontrado){
            pac=consultarColaPri();
            if(strcmp(nombre,pac.nombre)==0 && strcmp(apellido,pac.apellidos)==0){ //si nombre y apellidos encontrados
                encontrado=true;
                prioridad=priori(pac);
            }
            else{
                i=priori(pac);
                aux.insertarColaPri(i,pac);
